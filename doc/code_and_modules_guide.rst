...
For the overall design and math, see `solution_design.rst <solution_design.rst>`_.
For setup instructions and JSON files, see `run_and_configuration_guide.rst <run_and_configuration_guide.rst>`_.
...
* **Key Methods**:
  * ``select_format(operator_id, derived_tier, fatigue_index=0.0) -> Tuple[str, str]``:
    Checks the skill state. If the Fatigue Index is $\ge 0.80$, it sets exploration ($c$) to 0.0 (uses the most proven format). Otherwise, it calculates UCB1 scores for all formats (Visual, Terse, Detailed) and returns the winner.
